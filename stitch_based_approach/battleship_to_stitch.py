#!/usr/bin/env python3
"""
Convert Battleship programs to valid Stitch lambda calculus format.

Key constraints:
- Use De Bruijn indices (0, 1, 2...) for lambda variables
- Only simple primitives (no hyphens or special chars)
- Stitch format: (lam body), (app f x), primitives
"""
import ast
import json
import sys
...
class StitchConverter(ast.NodeVisitor):
    """Convert Python AST to Stitch format."""

    def __init__(self):
        self.var_stack: List[str] = []  # Track variable names for De Bruijn
    ...
    def visit_FunctionDef(self, node):
        ...
        # params = [a, b, c] -> (lam (lam (lam body)))
        # where body uses De Bruijn indices
        self.var_stack = params.copy()
    ...
    def visit_Name(self, node):
        ...
        # Clean up name (no special chars)
    ...
    def visit_Attribute(self, node):
        """Visit attribute access - just use the object."""
        # For things like arr.shape, just return arr
        return self.visit(node.value)
...
def convert_program(code: str) -> Optional[str]:
...
def main():
    ...
    output_file = 'battleship_stitch.json'

    programs = []
    successful = 0
    failed = 0

    print("Converting Battleship programs to Stitch format...")
    print()
    ...
    sample_file = 'sample_battleship_100.json'
    with open(sample_file, 'w') as f:
        json.dump(programs[:100], f, indent=2)
